...
import AccessoriesIteams from "./AccessoriesIteams";

const Accessories = () => {
    // const {accessories_name, discountPrice, price, accessories_image} = tem
    const [loading, setLoading] = useState(true);
    const [tem, setTem] = useState([])
    useEffect(()=>{
      fetch('/public/petitems.json')
      .then(res=> res.json())
      .then(data=>{
      setTem(data);
      setLoading(false);
  ...
  // Loading
  if (loading) return <p><span className="loading loading-ring loading-xl"></span></p>;


  return (
    <div>
      <div>
        <h1 className="text-2xl mb-8 font-semibold">Pet Food And Accessories</h1>
      <img
      ...
      <h1>hello:{tem.length}</h1>
        <div>
          {
            tem.map((loadsData)=> (<AccessoriesIteams give={loadsData}></AccessoriesIteams>))
          }
        </div>
        {/* Card iteam */}
